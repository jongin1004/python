...
def scrape_weather():
    ...
    morning_rain_rate = soup.find(
        "span", attrs={"class": "point_time morning"}).get_text().strip()  # 오전 강수
    afternoon_rain_rate = soup.find(
        "span", attrs={"class": "point_time afternoon"}).get_text().strip()  # 오후 강수
    ...
    # 조건은 여러개 줄 수 있다.
    # dust = soup.find("dl", attrs={"class" : "indicator", "class" : "name"}, text=["name1", "name2"])
...
def scrape_it_new():
    ...
    for idx, news in enumerate(news_list):
        a_idx = 0
        img = news.find_all("dt")
        if len(img) >= 2:
            a_idx = 1  # img 태그가 있으면 1번째 a 태그의 정보를 사용

        a_tag = news.find_all("dt")[a_idx].find("a")
...
def scrape_english():
    ...
    print("(영어 지문)")
    # 8문장이 있다고 가정할 때, 5~8까지 잘라서 가져옴 인덱스 기준 4~7
...
# main일 떄만 동작하게 해라
# 다른 파일에 의해서 호출될 경우에는 동작하지 않도록
if __name__ == "__main__":
    scrape_weather()  # 오늘의 날씨 정보 가져오기
    scrape_headline_new()  # 헤드라인 뉴스 정보 가져오기
    scrape_it_new()  # it 뉴스 정보 가져오기
